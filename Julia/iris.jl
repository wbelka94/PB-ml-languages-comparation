...
train =  CSV.read("C:/Users/Administrator/Desktop/PB/datasets/iris_train.csv" ; delim=',', header=true)
test =  CSV.read("C:/Users/Administrator/Desktop/PB/datasets/iris_test.csv" ; delim=',', header=true)

# print(first(train,5))
# ScikitLearn.jl expects arrays, but DataFrames can also be used - see
# the corresponding section of the manual
train_X = convert(Array, train[:,2:5])
train_y = convert(Array, train[:,6])
train_X = float.(train_X)
test_X = convert(Array, test[:,2:5])
test_y = convert(Array, test[:,6])
test_X = float.(test_X)
